...
Before beginning this lab, ensure the following prerequisites are in place:

1. Windows Server 2022 VM is running and promoted to a **Domain Controller** with AD DS installed.
2. The Windows 11 client VM is **joined to the `mylab.local` domain**.
3. Both VMs are on the **same virtual network** and can communicate.
4. You are logged into the Domain Controller as **Administrator**.
...
![Opening ADUC from Start Menu](screenshots/01-open-aduc.png)
...
![ADUC Domain Structure](screenshots/02-aduc-domain-view.png)
...
![Creating a New User Object](screenshots/03-aduc-new-user.png)
...
![User Creation Details](screenshots/04-user-creation-details.png)
...
![User Password Options](screenshots/05-user-password-options.png)
...
![User Creation Confirmation](screenshots/06-user-creation-confirm.png)
...
![Opening Group Policy Management](screenshots/07-open-gpo.png)
...
![GPO Account Lockout Policy Configured](screenshots/08-gpo-lockout-policy-final.png)
...
> ⚠️ **Why 3?** In a real environment, 5–10 attempts is more user-friendly. We use 3 here to make the lab quick and repeatable.
...
![Running gpupdate /force](screenshots/13-gpupdate-force.png)

The command output confirms the policy refresh is processing:

![gpupdate Output](screenshots/14-gpupdate-output.png)
...
![Locating Ben Tenison in ADUC](screenshots/16-locate-user-aduc.png)
...
![Right-click Properties](screenshots/17-rightclick-properties.png)
...
![Account Lock State in Properties](screenshots/18-account-locked-properties.png)
...
![Forced Password Change Prompt](screenshots/21-force-password-change-prompt.png)
...
![Password Changed Successfully](screenshots/22-password-changed-success.png)
...
![AD Password Change Confirmation](screenshots/23-ad-password-changed-confirm.png)
...
- **Time sensitivity is high.** Users locked out before meetings, presentations, or shift starts create immediate business impact. Speed and accuracy are critical.
...
- **The "User must change password" flag protects everyone.** If an admin resets a password and does not force a change, the admin now knows the user's live credential — an unnecessary security risk. Always enforce the change.
...
|---|---|---|
| "Unlock account" checkbox not visible | Account is not locked; it may be disabled | Check if account is disabled instead — right-click → Enable Account |
...
*Lab completed and documented by [Your Name] — IT Support Portfolio | GitHub: [your-username]*

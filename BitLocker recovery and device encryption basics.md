# Lab 19: BitLocker Recovery and Device Encryption Basics
...
## BitLocker Recovery Support Stack — Key Concept

> **BitLocker recovery events follow a strict three-layer process. Skipping any layer creates a security risk or a failed recovery.**

```
Layer 3 — User Identity Verification (before touching any key)
  Verify the caller is who they say they are
  "Can I confirm your employee ID, manager name, and the asset tag on the device?"
       │ Identity not confirmed → do not proceed, escalate to manager
       │ Identity confirmed → move to key retrieval
       ▼

Layer 2 — Recovery Key Retrieval (from approved source only)
  Azure AD: Endpoint Manager > Devices > find device > Recovery Keys tab
  Active Directory: Active Directory Users and Computers > BitLocker Recovery
  "Is the key for this specific device stored in our approved system?"
       │ Key found → retrieve and read to user via secure channel
       │ Key not found → escalate — do not guess or use a key from another device
       ▼

Layer 1 — Key Entry and Access Restoration (guided, not remote)
  Guide the user to type the 48-digit key on the recovery screen
  "Does Windows load normally after key entry?"
       │ Access restored → confirm, close ticket, advise on next steps
       │ Still locked → check if correct key was retrieved, escalate if key fails
```

A BitLocker recovery event is not a password reset. The key is unique to the device and the specific encryption state. Using a key from a different device or a key backed up before a hardware change can cause permanent data loss if the drive is decrypted incorrectly. Every step must be confirmed before proceeding.
...
![01 BitLocker Drive Encryption page — C drive showing BitLocker off](screenshots/01_bitlocker_off_baseline_annotated.png)

Pre-encryption state recorded:

| Item | Value | Significance |
|---|---|---|
| Drive | C: (Operating system drive) | This is the drive that protects Windows and all user data |
| BitLocker Status | **Off** | Drive is unencrypted — data accessible to anyone with physical access |
| Option available | Turn on BitLocker | Encryption not yet configured |
| Fixed data drives | None shown | No secondary data drives present in this VM |
...
Click **Turn on BitLocker** next to the C: drive. BitLocker will ask how you want to back up the recovery key before encryption begins. This step is critical — if the recovery key is lost, the encrypted drive cannot be unlocked and all data is permanently inaccessible.

![02 Recovery key backup options — Save to a file and Print options shown](screenshots/02_recovery_key_backup_options_annotated.png)

Recovery key backup options presented:

| Option | Use Case | Lab Choice |
|---|---|---|
| Save to Microsoft account | Personal devices — stores key in Microsoft cloud | Not used |
| Save to a file | Test environments and manual backup workflows | **Selected for this lab** |
| Print the recovery key | Physical backup — store securely offline | Not used |
| Save to Azure AD | Enterprise managed devices — recommended for business | Not available in this VM |
| Save to Active Directory | Domain-joined devices — stored against computer object | Not available in this VM |
...
![03 BitLocker encryption in progress — 66.3 percent complete](screenshots/03_encryption_in_progress_annotated.png)

> **Highlighted:** The encryption is at 66.3% complete. The progress bar shows the encryption actively writing to the drive. This process runs in the background and the machine remains usable during encryption. The drive is considered protected from the moment encryption begins — any data already written is encrypted, and new data is encrypted as it is written.

> Do not shut down the machine or disconnect power during encryption. If this occurs in a lab or production environment, BitLocker will resume encryption on the next boot — it does not restart from zero. However, an interrupted encryption on a failing drive can cause data loss.
...
![04 BitLocker encryption complete — Encryption of C: is complete](screenshots/04_encryption_complete_annotated.png)

> **Highlighted:** The message reads Encryption of C: is complete. The drive is now fully encrypted. From this point, all data on C: is protected by the BitLocker key. Without the PIN, TPM chip, or recovery key, the drive cannot be read by any external system.
...
![05 Recovery key file saved to VM shared folder — filename redacted for security](screenshots/05_recovery_key_saved_location_annotated.png)

> **Security note — Screenshot handling:** The filename of the recovery key file contains a partial Key ID (a unique identifier that matches the BitLocker key on the drive). For this reason the filename has been redacted in the annotated screenshot above. The actual 48-digit recovery key was never captured in any screenshot. This is the correct practice for any documentation published to a public repository.

Key file storage confirmed:

| Item | Detail |
|---|---|
| File type | Text Document (.txt) |
| File size | 2 KB (consistent with a BitLocker key file) |
| Save location | Z:\VM_SHARED_FOLDER — external to C: |
| Date saved | 4/5/2026 9:49 AM — recorded for audit trail |
| Key ID in filename | Redacted — not published |

> In a business environment this file would not exist. The key would be stored in Azure AD or Active Directory and retrieved by the technician through the admin console. The file-based backup used in this lab is a simulation of the workflow — not a recommended enterprise practice.
...
![06 BitLocker on confirmed — C drive protected, management options visible](screenshots/06_bitlocker_on_confirmed_annotated.png)

Post-encryption state recorded:

| Item | Before | After |
|---|---|---|
| BitLocker status | Off | **On** |
| Drive protection | Unencrypted | Fully encrypted |
| Options available | Turn on BitLocker | Suspend protection, Back up recovery key, Turn off BitLocker |
| Admin policy notice | Not shown | Shown — some settings managed by system administrator |
...
>
> The admin policy notice (shown in orange) states that some settings are managed by the system administrator. This appears because the VM is running a Windows 11 build where certain BitLocker policies are pre-configured. In a business environment this is expected — Group Policy or Intune controls BitLocker settings to ensure consistent encryption standards across the fleet.
...
The screen shows the text Enter the recovery key for this drive. The user can read the first 8 characters of the Key ID shown on screen (for example: 54B8DD9A). This Key ID is used to match against the stored key in the admin system — it is not the key itself.
